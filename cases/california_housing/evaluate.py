"""Evaluación de los modelos del caso California Housing.

Calcula las métricas de regresión (MAE, MSE, RMSE, R²) y genera las
visualizaciones académicas del caso.
"""
...
def plot_target_distribution(df: pd.DataFrame, path: Path) -> None:
    ...
    sns.histplot(df["median_house_value"], bins=50, kde=True, color="#2E86AB")
    plt.title("Distribución de median_house_value")
    plt.xlabel("Valor medio de la vivienda (USD)")
...
def plot_actual_vs_predicted(y_test, y_pred, title: str, path: Path) -> None:
    ...
    sns.scatterplot(x=y_test, y=y_pred, alpha=0.4, s=18, color="#2E86AB")
...
def plot_residuals(y_test, y_pred, title: str, path: Path) -> None:
    ...
    plt.figure(figsize=(8, 6))
    sns.scatterplot(x=y_pred, y=residuals, alpha=0.4, s=18, color="#2E86AB")
    plt.axhline(0, color="#C0392B", linestyle="--")
    plt.xlabel("Valores predichos (USD)")
