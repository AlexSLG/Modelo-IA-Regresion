"""Entrenamiento y guardado de los modelos del caso California Housing.
...
def build_models(preprocessor) -> dict:
    ...
    return {
        ...
        "polynomial_degree_2": Pipeline(
            steps=[
                ("pre", preprocessor),
                # Expande las características a términos de grado 2 (y sus interacciones).
        ...
        "polynomial_degree_3": Pipeline(
            steps=[
                ("pre", preprocessor),
                # Grado 3: captura relaciones más complejas con mayor riesgo de sobreajuste.
...
def compute_input_ranges(df: pd.DataFrame) -> dict:
    ...
    return {
        ...
        "categorical": {
            "ocean_proximity": df["ocean_proximity"].unique().tolist()
        },
        "target": {
            "name": TARGET,
